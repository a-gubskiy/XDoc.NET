...
internal class ParsedContentResolver
{
    ...
    public static MemberDocumentation Resolve(MemberDocumentation documentation)
    {
        return new ParsedContentResolver(documentation).Resolve();
    }

    private MemberDocumentation Resolve()
    {
        var references = GetReferences();

        throw new NotImplementedException();

        // return new MemberDocumentation(_documentation, references);
    }

    private IReadOnlyDictionary<string, MemberDocumentation?> GetReferences()
    {
        var references = new Dictionary<string, MemberDocumentation?>();
        ...
        foreach (var typeName in declaredReferences)
        {
            ...
            if (type != null)
            {
                ...
                // new MemberDocumentation(documentation, internalReferences);
    ...
    public static MemberDocumentation? GetInheritedContent(MemberDocumentation documentation)
    {
    ...
    private MemberDocumentation? GetInheritedContentByTarget(MemberDocumentation documentation)
    {
        throw new NotImplementedException();
    }

    private MemberDocumentation? GetParentInheritedContent()
    {
        throw new NotImplementedException();
    }
    ...
    private static MemberDocumentation? GetInheritedContent(XmlNode? xmlNode, XDoc xDoc, MemberInfo memberInfo)
    {
        ...
        foreach (var parent in parentTypes)
        {
            ...
            foreach (var parentMember in parentMembers)
            {
                if (parentMember is PropertyInfo parentPropertyInfo)
                {
                    ...
                    // return new MemberDocumentation(GetReferences(parentPropertyDocumentation?.Node, xDoc))
                    // {
